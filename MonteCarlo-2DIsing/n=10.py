...
def plot_magnetization(dimension):
    ...
    for T in T_list:
        ...
        fig.suptitle(f"Single Flip Monte Carlo Simulation (T = {T:.3f}) - {dimension}D Ising Model", fontsize=14)
...
def plot_magnetization_cluster(dimension):
    ...
    for T in T_list:
        ...
        # Create a new figure for each temperature
        fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 4))
        fig.suptitle(f"Cluster Monte Carlo Simulation (T = {T}) - {dimension}D Ising Model", fontsize=14)
